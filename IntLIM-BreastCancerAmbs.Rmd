...
write.csv(myres10@corr, "bcresults10.csv")
dim(myres10@corr)
```
...
write.csv(myres@corr, "bcresults5.csv")
dim(myres@corr)
```
...
We will cut the heatmap as below. We find 1038 gene-metabolite pairs in the Tumor-correlated cluster and 1804 pairs in the Tumor anti-correlated cluster.    
...
dim(cluster.1)
dim(cluster.2)
summary(cluster.1$TUMOR)
summary(cluster.2$TUMOR)
...
We need to fit the metabolite data to HMDB IDs
